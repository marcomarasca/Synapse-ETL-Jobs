"""
This script executed by a Glue job for back-filling the deleted node snapshots. The job take the node snapshot data from S3
which is in csv format and process it. Processed data stored in S3 in a json format and partitioned by change timestamp of
...
input_frame = glueContext.create_dynamic_frame.from_options(
    ...
    format="csv",
    connection_options={
        "paths": [args["S3_SOURCE_PATH"] + args["RELEASE_NUMBER"] + "/deletednode/"],
        "recurse": True,
    },
